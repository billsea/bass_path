...
- (void)awakeFromNib {
  // NSString *path = [[NSBundle bundleForClass: [bassAmp_CocoaView class]]
  ...
  mBackgroundColor = [NSColor whiteColor]; //[[NSColor colorWithPatternImage:
                                           //[pattern autorelease]] retain];
  backgroundImage = [NSImage imageNamed:@"brushedMetal.gif"];

  // setup a timer to call limiter meter display
  gLimiterMeterTimer = [[NSTimer
      scheduledTimerWithTimeInterval:0.01 // interval, 0.01 seconds
                              target:self
                            selector:@selector(
                                         displayLimiterGR:) // call this method
                            userInfo:nil
...
- (void)evaluationCheck {
  ...
  NSString *regPath =
      [[NSBundle bundleWithIdentifier:@"com.primomedia.audiounit.bassAmp"]
          pathForResource:@"rgLic"
                   ofType:@"plist"];
  ...
  NSString *serialNum = [[NSString alloc]
      initWithString:
          @"bampv100x757832"]; // this is also set in
...
{
  if (!regCustomSheet)
    // Check the myCustomSheet instance variable to make sure the custom sheet
    // does not already exist.
    [NSBundle loadNibNamed:@"registerBP" owner:self];

  // Registration sheet is displayed, and application is suspended until the
...
- (IBAction)serialNumberEntry:(id)sender {
  NSString *serialNum = [[NSString alloc]
      initWithString:@"bampv100x757832"]; // valid registration number
  if ([[regNumberField stringValue] isEqual:serialNum]) {
    ...
    [regDict writeToFile:regPath
              atomically:nil]; // write back to rgLic.plist file

    int serNum = NSRunAlertPanel(@"Registration completed!",
                                 @"Thanks for Registering bassPath!", nil, nil,
                                 nil, nil);
    ...
    int serNum = NSRunAlertPanel(@"Registration entry error",
                                 @"The registration number you have entered is "
                                 @"not valid, please confirm your "
                                 @"registration number re-enter.",
                                 nil, nil, nil, nil);
  }
}
...
- (IBAction)unloadApp:(id)sender;
{
  ...
  int serNum = NSRunAlertPanel(
      @"Thanks for evaluating bassPath!",
      @"bassPath will now run in bypass mode. Please register today!", nil, nil,
      nil, nil);
...
- (void)drawRect:(NSRect)rect {
  ...
  //[backgroundImage compositeToPoint:NSMakePoint(0.0,0.0)
  //operation:NSCompositeSourceOver];
  NSRectFill(rect); // this call is much faster than using NSBezierPath, but it
                    // doesn't handle non-opaque colors

  [super drawRect:rect]; // we call super to draw all other controls after we
                         // have filled the background
}
...
- (IBAction)iaOnStatus:(id)sender {
  ...
  NSAssert(AUParameterSet(mParameterListener, sender, &parameter[0], intValue,
                          0) == noErr,
...
- (IBAction)iaParam1Changed:(id)sender {
  ...
  NSAssert(AUParameterSet(mParameterListener, sender, &parameter[1],
...
- (IBAction)iaParam2Changed:(id)sender {
  float floatValue = [sender getKnobLinearValue];
  NSAssert(AUParameterSet(mParameterListener, sender, &parameter[2],
                          (Float32)floatValue, 0) == noErr,
...
- (IBAction)iaParam3Changed:(id)sender {
  float floatValue = [sender getKnobLinearValue];
  NSAssert(AUParameterSet(mParameterListener, sender, &parameter[3],
                          (Float32)floatValue, 0) == noErr,
...
- (IBAction)iaParam4Changed:(id)sender {
  float floatValue = [sender getKnobLinearValue];
  NSAssert(AUParameterSet(mParameterListener, sender, &parameter[4],
                          (Float32)floatValue, 0) == noErr,
...
- (IBAction)iaParam5Changed:(id)sender {
  float floatValue = [sender getKnobLinearValue];
  NSAssert(AUParameterSet(mParameterListener, sender, &parameter[5],
                          (Float32)floatValue, 0) == noErr,
...
- (IBAction)iaParam6Changed:(id)sender {
  float floatValue = [sender getKnobLinearValue];
  NSAssert(AUParameterSet(mParameterListener, sender, &parameter[6],
                          (Float32)floatValue, 0) == noErr,
...
- (IBAction)iaParam7Changed:(id)sender {
  float floatValue = [sender getKnobLinearValue];
  NSAssert(AUParameterSet(mParameterListener, sender, &parameter[7],
                          (Float32)floatValue, 0) == noErr,
...
- (IBAction)iaParam8Changed:(id)sender {
  float floatValue = [sender getKnobLinearValue];
  NSAssert(AUParameterSet(mParameterListener, sender, &parameter[8],
                          (Float32)floatValue, 0) == noErr,
...
- (IBAction)iaParam9Changed:(id)sender {
  float floatValue = [sender getKnobLinearValue];
  NSAssert(AUParameterSet(mParameterListener, sender, &parameter[9],
                          (Float32)floatValue, 0) == noErr,
...
- (IBAction)iaParam10Changed:(id)sender {
  float floatValue = [sender getKnobLinearValue];
  NSAssert(AUParameterSet(mParameterListener, sender, &parameter[10],
                          (Float32)floatValue, 0) == noErr,
  ...
  if (sender == uiParam10knob) {
    [uiParam1TextField
        setStringValue:[NSString
                           stringWithFormat:@"%.3f", [theValue floatValue]]];
  } else {
...
- (IBAction)iaParam11Changed:(id)sender {
  float floatValue = [sender getKnobLinearValue];
  NSAssert(AUParameterSet(mParameterListener, sender, &parameter[11],
                          (Float32)floatValue, 0) == noErr,
...
- (IBAction)iaParam12Changed:(id)sender {
  float floatValue = [sender getKnobLinearValue];
  NSAssert(AUParameterSet(mParameterListener, sender, &parameter[12],
                          (Float32)floatValue, 0) == noErr,
...
- (IBAction)iaParam13Changed:(id)sender {
  float floatValue = [sender getKnobLinearValue];
  NSAssert(AUParameterSet(mParameterListener, sender, &parameter[13],
                          (Float32)floatValue, 0) == noErr,
...
- (IBAction)iaParam14Changed:(id)sender {
  float floatValue = [sender getKnobLinearValue];
  NSAssert(AUParameterSet(mParameterListener, sender, &parameter[14],
                          (Float32)floatValue, 0) == noErr,
...
- (IBAction)iaParam15Changed:(id)sender {
  ...
  NSAssert(AUParameterSet(mParameterListener, sender, &parameter[15], intValue,
                          0) == noErr,
...
// called by timer(initialized in awakeFromNIB function) to display limiter gain
// reduction
- (void)displayLimiterGR:(id)sender;
{
  float value;
  ComponentResult cr;
  cr = AudioUnitGetParameter(mAU, parameter[16].mParameterID,
...
- (void)_addListeners {
  ...
  for (i = 0; i < kNumberOfParameters; ++i) {
    ...
    NSAssert(AUListenerAddParameter(mParameterListener, NULL, &parameter[i]) ==
                 noErr,
...
- (void)_removeListeners {
  ...
  for (i = 0; i < kNumberOfParameters; ++i) {
    NSAssert(
        AUListenerRemoveParameter(mParameterListener, NULL, &parameter[i]) ==
            noErr,
...
- (void)_synchronizeUIWithParameterValues {
  ...
  for (i = 0; i < kNumberOfParameters; ++i) {
    ...
    NSAssert(AudioUnitGetParameter(mAU, parameter[i].mParameterID,
                                   kAudioUnitScope_Global, 0, &value) == noErr,
             @"[bassAmp_CocoaView synchronizeUIWithParameterValues] (x.1)");
    NSAssert(AUParameterSet(mParameterListener, self, &parameter[i], value,
                            0) == noErr,
...
- (void)_parameterListener:(void *)inObject
  ...
  if ([self getAdjustState] != 1) {
    ...
    case kParam_One_Indexed:
      ...
      //[uiParam1TextField setStringValue:[[NSNumber numberWithInt:inValue]
      //stringValue]];
      break;
    case kParam_Two:
      ...
      [uiParam1TextField
          setStringValue:[[NSNumber numberWithFloat:inValue] stringValue]];
      break;
    case kParam_Three:
      // NSLog(@"inValue3 value = %f", inValue);
      [uiParam2knob setCurrentValue:inValue];
      //[uiParam1TextField setStringValue:[[NSNumber numberWithFloat:inValue]
      //stringValue]];
      break;
    case kParam_Four:
