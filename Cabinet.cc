...
CabinetI::switch_model (int m)
{
	...
	gain = models[m].gain * DSP::db2lin (24.0); //  5.0 is the gain setting
...
CabinetI::one_cycle (int frames)
{
	...
	d_sample g = models[model].gain * DSP::db2lin (24.0); //5.0 is the gain setting
	double gf = pow (g / gain, 1 / (double) frames);

	d_sample * d = &input;//ports[3];//ports[3] output
