...
void pm_Delay::processInput(Float32 &srcInL, Float32 &srcInR) {
  ...
  if (frames1 < 1)
    frames1 = 1;
  else if (frames1 >= pmMaxDelayFrames)
    frames1 = pmMaxDelayFrames - 1;

  level = phaseGain;
  readIndex =
      pNormalizeIndex(pmWriteIndex - frames1); // read head lags write head...
...
////////////////////////////
void pm_Delay::setWriteIndex(int wIndex) { pmWriteIndex = wIndex; }

int pm_Delay::getWriteIndex() { return pmWriteIndex; }

int pm_Delay::pNormalizeIndex(int i) {
  return (i + pmMaxDelayFrames) % pmMaxDelayFrames;
}

void pm_Delay::setSampleRate(double sampleRate) { sampRate = sampleRate; }

void pm_Delay::setDelayTime(Float32 delTime) { delayTime = delTime; }
void pm_Delay::setWetDryLevel(Float32 level) { wdLevel = level; }
}
