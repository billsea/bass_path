...
pmOverdrive::pmOverdrive(int sampRate)
{
	m_SampleRate = sampRate;
}

void pmOverdrive::setSampleRate(float sampRate)
{
	m_SampleRate = sampRate;
}
...
void pmOverdrive::Distort(Float32 &in1, float amount)
{
	if (amount > 0){
		...
		float inHPF = tube.transfer_clip(hp_filter(tbtrans,100,-(amount * 5)));// add HP filter

		if (amount>=0.99) amount = 0.99;
	
		float k=8*amount/(1-amount); //float k=2*amount/(1-amount); original
...
void pmOverdrive::Overdrive(Float32 &in1, float amount)
{
	if (amount > 0){
		...
		in1 = ( ((1+k) * inHPF/(1+k*fabs(inHPF)))*(1-amount) ) + (in1 * 0.05); ;//add unprocessed signal
	}
...
// highpass filter
////////////////////////////////////////////////////////
Float32 pmOverdrive::hp_filter(Float32 input, Float32 trebCor, Float32 cutBoost)
{
